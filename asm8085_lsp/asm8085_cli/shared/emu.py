...
# Simple structure to hold error details
class ErrorInfo:
    def __init__(self, message: str, line_number: int, column: int = 0):
        self.message = message
        self.line_number = line_number
        self.column = column  # TODO: Implement column detection

    def __str__(self):
        return f"[L:{self.line_number}] {self.message}"


# Stub for PluginExternal - not needed in CLI mode
class PluginExternal:
    """Plugin for console I/O in CLI mode."""

    def __init__(self):
        self.isconnected = False
        # Buffer for stdin to handle non-blocking reads if needed,
        # but for now we'll do blocking reads.
        self.input_buffer = []

    def tryconnect(self, port: int = 6772) -> bool:
        # In CLI mode, we are always "connected" to the console.
    ...
    def inport(self, port: int) -> Tuple[bool, int]:
        ...
        if port == 0x00:
            try:
                ...
                return False, 0
        return False, 0xFF  # Return FF for other ports
...
class emu8085:
    ...
    def getcurrentline(self) -> int:
        if self.haulted == False:
            try:
                ...
                print(
                    "exeception encountered while getting current binary line program ran out of scope!"
                )
                self.wasexecerr = True
                self.haulted = True
                return 0
    ...
    def runcrntins(self):
        ...
        # sub r
            ...
            self.setsignflag(fval)
            self.setzeroflag(fval)
            self.setauxicarryflag(
                self.iscaseauxcarry(self.ln2comp(opval), self.A.value)
            )
            self.setparityflag(fval)
        ...
        # dcr r
            ...
            if sreg == 0x0:
                fval = self.B.value - 0x01
                self.setauxicarryflag(
                    self.iscaseauxcarry(self.ln2comp(0x01), self.B.value)
                )
                self.B.value = fval
            elif sreg == 0x1:
                fval = self.C.value - 0x01
                self.setauxicarryflag(
                    self.iscaseauxcarry(self.ln2comp(0x01), self.C.value)
                )
                self.C.value = fval
            elif sreg == 0x2:
                fval = self.D.value - 0x01
                self.setauxicarryflag(
                    self.iscaseauxcarry(self.ln2comp(0x01), self.D.value)
                )
                self.D.value = fval
            elif sreg == 0x3:
                fval = self.E.value - 0x01
                self.setauxicarryflag(
                    self.iscaseauxcarry(self.ln2comp(0x01), self.E.value)
                )
                self.E.value = fval
            elif sreg == 0x4:
                fval = self.H.value - 0x01
                self.setauxicarryflag(
                    self.iscaseauxcarry(self.ln2comp(0x01), self.H.value)
                )
                self.H.value = fval
            elif sreg == 0x5:
                fval = self.L.value - 0x01
                self.setauxicarryflag(
                    self.iscaseauxcarry(self.ln2comp(0x01), self.L.value)
                )
                self.L.value = fval
            elif sreg == 0x6:
                fval = self.getM() - 0x01
                self.setauxicarryflag(
                    self.iscaseauxcarry(self.ln2comp(0x01), self.getM())
                )
                self.setM(fval)
            elif sreg == 0x7:
                fval = self.A.value - 0x01
                self.setauxicarryflag(
                    self.iscaseauxcarry(self.ln2comp(0x01), self.A.value)
                )
                self.A.value = fval
            self.setsignflag(fval)
            self.setzeroflag(fval)
...
class assembler:
    ...
    def addtooresolvelabel(self, label: str, offset: int, line_number: int) -> None:
        if label not in self.toresolvelabels:
            self.toresolvelabels[label] = []
        self.toresolvelabels[label].append((offset, line_number))

    def addlabeloff(self, label: str, offset: int) -> bool:
        if label not in self.labeloff:
    ...
    def miscissingledarg(self, lexa, off) -> list[bool, str]:
        if len(lexa) == (off + 1) + 1:
            if (
                lexa[off + 1] == LexTag.DSHORT
                or lexa[off + 1] == LexTag.DDSHORT
                or lexa[off + 1] == LexTag.DDBYTE
                or lexa[off + 1] == LexTag.DSTRING
            ):
                return True, ""
            else:
    ...
    def miscissinglelab(self, lexa, off) -> list[bool, str]:
        if len(lexa) == (off + 1) + 1:
            if lexa[off + 1] == LexTag.DSTRING or lexa[off + 1] == LexTag.DSHORT:
                return True, ""
            else:
    ...
    def miscissinglerparg(
        self, lexa, s, off, regs=["B", "D", "H", "SP"]
    ) -> list[bool, str]:
        if len(lexa) == (off + 1) + 1:
            if s in regs:
                return True, ""
    ...
    def resolvelabels(self) -> Tuple[bool, Union[str, ErrorInfo]]:
        ...
        for label in list(self.labeloff):
            if label in list(self.toresolvelabels):
                ...
                adl = memoff >> 8
                adu = memoff & 0x00FF
                for toupoff, line_num in self.toresolvelabels[label]:
                    self.pmemory[toupoff] = adu
                    self.pmemory[toupoff + 1] = adl
                self.toresolvelabels.pop(label)
        if self.toresolvelabels:
            # Get the first unresolved label and one of its line numbers
            failed_label = list(self.toresolvelabels.keys())[0]
            line_number = self.toresolvelabels[failed_label][0][1]
            return False, ErrorInfo(f"Unresolved label '{failed_label}'", line_number)
        return True, "success!"
    ...
    def assemble(self, lines) -> Tuple[bool, Union[str, ErrorInfo]]:
        self.dclines = lines
        for line_num, line in enumerate(lines):
            ...
            if sa != []:
                if LexTag.ASMDIR in lexa:
                    # Check if there's a label before the directive (label: DIRECTIVE)
                    dir_offset = 0
                    if (
                        len(lexa) >= 2
                        and lexa[0] == LexTag.DSTRING
                        and lexa[1] == LexTag.SCOLON
                    ):
                        # Label found, add it and skip to directive
                        if (
                            self.addlabeloff(sa[0], self.cprogmemoff + self.ploadoff)
                            == False
                        ):
                            return False, ErrorInfo(
                                f"label {sa[0]} was already defined", line_num
                            )
                        dir_offset = 2

                    if lexa[dir_offset] != LexTag.ASMDIR:
                        return False, ErrorInfo(
                            "was expecting directive first", line_num
                        )
                    elif sa[dir_offset] == "ORG":
                        if (
                            len(lexa) != dir_offset + 2
                            or lexa[dir_offset + 1] != LexTag.DSHORT
                        ):
                            return False, ErrorInfo("invalid args", line_num)
                        self.ploadoff = int(sa[dir_offset + 1][:-1], 16)
                        self.cprogmemoff = 0
                    elif sa[dir_offset] == "DB":
                        status, msg, vals = self.getdbarray(
                            lexa[dir_offset + 1 :], sa[dir_offset + 1 :]
                        )
                        if status == False:
                            return status, ErrorInfo(msg, line_num)
                        for val in vals:
                            opcodes.append(val)
                            oplen += 1
                    elif sa[dir_offset] == "DS":
                        status, msg, vals = self.getdsarray(
                            lexa[dir_offset + 1 :], sa[dir_offset + 1 :]
                        )
                        if status == False:
                            return status, ErrorInfo(msg, line_num)
                    ...
                    for lex in lexa:
                        if lex == LexTag.SCOLON:
                            scount += 1
                    if scount > 1:
                        return False, ErrorInfo("was expecting single colon", line_num)
                    if lexa[0] == LexTag.DSTRING and lexa[1] == LexTag.SCOLON:
                        ...
                        return False, ErrorInfo(
                            "was expecting string before colon", line_num
                        )
                if inso != -1:
                    if lexa[inso] != LexTag.OPCODE:
                        return False, ErrorInfo("was expecting a opcode!", line_num)
                    else:
                        # type no arg
                        ins = sa[inso]
                        if ins in list(_inc_narg):
                            ...
                            if b == False:
                                return False, ErrorInfo(m, line_num)
                        ...
                        # type single reg with offset relation t1 arg
                            ...
                            if b == False:
                                return False, ErrorInfo(m, line_num)
                            opcodes.append(
                                self.miscopcodertoff(_inc_srt1arg[ins], sa[inso + 1])
                            )
                            oplen = 1
                        # type single reg with offset relation t2 arg
                            ...
                            if b == False:
                                return False, ErrorInfo(m, line_num)
                            opcodes.append(
                                self.miscopcodertoff(_inc_srt2arg[ins], sa[inso + 1], 8)
                            )
                            oplen = 1
                        # type single reg pair with offset relation t3 arg
                        elif ins in list(_inc_srpt3arg):
                            b, m = self.miscissinglerparg(
                                lexa, sa[inso + 1], inso, _inc_srpt3regs
                            )
                            if b == False:
                                return False, ErrorInfo(m, line_num)
                            opcodes.append(
                                self.miscopcoderpt3off(
                                    _inc_srpt3arg[ins], sa[inso + 1], _inc_srpt3regs, 16
                                )
                            )
                            oplen = 1
                        # type single reg pair with offset relation t4 arg
                            ...
                            b, m = self.miscissinglerparg(
                                lexa, sa[inso + 1], inso, _inc_srpt4regs
                            )
                            if b == False:
                                return False, ErrorInfo(m, line_num)
                            opcodes.append(
                                self.miscopcoderpt3off(
                                    _inc_srpt4arg[ins], sa[inso + 1], _inc_srpt4regs, 16
                                )
                            )
                            oplen = 1
                        # type single reg pair with offset relation t5 arg
                            ...
                            b, m = self.miscissinglerparg(
                                lexa, sa[inso + 1], inso, _inc_srpt5regs
                            )
                            if b == False:
                                return False, ErrorInfo(m, line_num)
                            opcodes.append(
                                self.miscopcoderpt3off(
                                    _inc_srpt5arg[ins], sa[inso + 1], _inc_srpt5regs, 16
                                )
                            )
                            oplen = 1
                        # type single lab arg
                            ...
                            if lexa[inso + 1] == LexTag.DSHORT:
                                ...
                                opcodes.append(0xCA)
                                opcodes.append(0xCA)
                                self.addtooresolvelabel(
                                    sa[inso + 1],
                                    self.ploadoff + self.cprogmemoff + 1,
                                    line_num,
                                )
                            oplen = 3
                        # type single double arg
                            ...
                            if lexa[inso + 1] == LexTag.DSHORT:
                                ...
                                opcodes.append(adl)
                                opcodes.append(adu)
                            elif lexa[inso + 1] == LexTag.DSTRING:
                                # Label - need to resolve it
                                self.addtooresolvelabel(
                                    sa[inso + 1],
                                    self.ploadoff + self.cprogmemoff + 1,
                                    line_num,
                        ...
                        # some distinct hardcoded
                            ...
                            if len(sa) != (inso + 1) + 3:
                                return False, ErrorInfo("not enough args", line_num)
                            if (
                                sa[inso + 1] in _inc_srpt3regs
                                and lexa[inso + 2] == LexTag.SCOMMA
                                and (
                                    lexa[inso + 3]
                                    in [LexTag.DSHORT, LexTag.DDSHORT, LexTag.DDBYTE]
                                )
                            ):
                                if lexa[inso + 3] == LexTag.DSHORT:
                                    ...
                                    opcodes.append(
                                        self.miscopcoderpt3off(
                                            0x01, sa[inso + 1], _inc_srpt3regs, 16
                                        )
                                    )
                                    opcodes.append(adu)
                                    opcodes.append(adl)
                            ...
                            if len(sa) != (inso + 1) + 3:
                                return False, ErrorInfo("not enough args", line_num)
                            if (
                                lexa[inso + 1] == LexTag.REG
                                and lexa[inso + 2] == LexTag.SCOMMA
                                and (lexa[inso + 3] in [LexTag.DBYTE, LexTag.DDBYTE])
                            ):
                                if lexa[inso + 3] == LexTag.DBYTE:
                                    d = int(sa[inso + 3][:-1], 16)
                                    opcodes.append(
                                        self.miscopcodertoff(0x06, sa[inso + 1], 8)
                                    )
                                    opcodes.append(d)
                                else:
                                    d = int(sa[inso + 3])
                            ...
                            if (
                                lexa[inso + 1] == LexTag.REG
                                and lexa[inso + 2] == LexTag.SCOMMA
                                and lexa[inso + 3] == LexTag.REG
                                and not (sa[inso + 1] == "M" and sa[inso + 3] == "M")
                            ):
                                topcode = self.miscopcodertoff(
                                    self.miscopcodertoff(0x40, sa[inso + 3]),
                                    sa[inso + 1],
                                    8,
                                )
                            ...
                            return False, ErrorInfo("opcode locate error", line_num)
            ...
            for opcode in opcodes:
                ...
                if self.writtenaddresses[self.ploadoff + self.cprogmemoff] != 0:
                    return False, ErrorInfo("memory override on same address", line_num)
